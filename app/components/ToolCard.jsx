...
export default function ToolCard({ tool, onAddToLibrary, adding }) {
  return (
    <div className="bg-white border border-gray-200 hover:border-blue-500 hover:shadow-lg transition group overflow-hidden relative z-10">
      ...
      <div className="p-4">
        ...
        <div className="flex justify-between items-start mb-3">
          ...
          {tool.brand && (
            <span className="text-xs bg-gray-100 text-gray-600 px-2 py-1 border border-gray-200 ml-2 whitespace-nowrap">
              {tool.brand}
            </span>
          )}
        </div>
        ...
        <div className="flex justify-between items-center pt-3 border-t border-gray-200">
          <div>
            {tool.category && (
              <span className="text-xs text-gray-400 block mb-1">{tool.category}</span>
            )}
            {tool.price ? (
              <span className="text-lg font-bold text-blue-600">${Number(tool.price).toFixed(2)}</span>
            ) : (
              <span className="text-sm text-gray-500">Request price</span>
            )}
          </div>
          <button
